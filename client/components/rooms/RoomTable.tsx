...
export default function RoomTable({ participants, scrambleIndex, userStatuses = {}, currentUserId }: RoomTableProps) {
    ...
    // Scroll to top on updates (since newest is at top)
    useEffect(() => {
    ...
    return (
        ...
        >
            <div className="flex flex-col min-w-full w-max md:w-full">
                ...
                {/* Header (Sticky) */}
                <div className="sticky top-0 z-20 flex w-full bg-[#1E1E1E] border-b border-[#333] text-xs font-bold uppercase tracking-wider text-gray-400">
                    ...
                    {participants.map(p => {
                        ...
                        return (
                            <div key={p.id} className={`flex-1 min-w-[100px] md:min-w-0 flex items-center justify-center py-3 px-2 border-r border-[#333] last:border-0 truncate ${isMe ? 'text-blue-200 bg-white/[0.03]' : 'text-gray-400'}`}>
                                {p.username}
                            </div>
                        );
                    })}
                ...
                <div className="flex w-full bg-[#252525] border-b border-[#333] font-mono text-gray-300">
                    ...
                    {participants.map(p => {
                        ...
                        return (
                            <div key={p.id} className={`flex-1 min-w-[100px] md:min-w-0 flex items-center justify-center py-2 border-r border-[#333] last:border-0 ${isBest ? 'text-emerald-400 font-bold' : ''}`}>
                                {stat?.meanStr}
                            </div>
                        );
                    })}
                ...
                <div className="flex w-full bg-[#252525] border-b border-[#333] font-mono text-gray-300 mb-[1px]">
                    ...
                    {participants.map(p => {
                        return (
                            <div key={p.id} className="flex-1 min-w-[100px] md:min-w-0 flex items-center justify-center py-2 border-r border-[#333] last:border-0 text-emerald-500/80">
                            ...
                            >
                                ...
                                {participants.map(p => {
                                    ...
                                    return (
                                        <div
                                            key={p.id}
                                            className={`flex-1 min-w-[100px] md:min-w-0 flex items-center justify-center py-1.5 border-r border-[#333] last:border-0`}
                                        >
                                            {cellContent}
                                        </div>
                                    );
